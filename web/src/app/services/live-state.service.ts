...
import { animationFrames, buffer, filter, Subject, take, takeUntil } from 'rxjs';
...
export class LiveStateService implements OnDestroy {
  ...
  readonly liveState = signal<Map<string, CharacterLiveState>>(new Map());
  ...
  setReplayMode(active: boolean): void {
    ...
    // Keep replayMode=true for one more animation frame so any events
    // still queued in the buffer(animationFrames()) pipe are discarded
    // before we open up for live events again.
  ...
  private processBatch(events: LiveEvent[]): void {
    ...
    for (const event of events) {
      ...
      updatedMap.set(charName, applyEvent(existing, event));
